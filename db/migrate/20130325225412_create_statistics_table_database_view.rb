class CreateStatisticsTableDatabaseView < ActiveRecord::Migration
  def up
    execute("DROP VIEW IF EXISTS statistics_table_v;")
    drop_table :statistics_table_v if self.table_exists?("statistics_table_v")
    execute(
  ...
  def down
    execute("DROP VIEW statistics_table_v;")
    drop_table :super_admin_table_v if self.table_exists?("statistics_table_v")
  end
end